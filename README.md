...
- **接入 TMDB**：添加电影 / 剧集时可「搜索 TMDB」自动补全标题、年份、简介、海报（v1.3 新增，详见下文）
...
添加记录时，标题框旁有「搜索 TMDB」按钮：输入片名后点击，会列出 TMDB 上匹配的**电影 / 剧集**（每条显示海报小图、标题、年份、类型）；点选一条即自动把**标题、年份、简介、海报图 URL** 填进表单，你仍可手动修改后再保存。列表和编辑页都会展示海报缩略图。

> 目前只做了电影 / 剧集（TMDB）。书（Open Library）、动画（Bangumi）后续再加。
...
- 密钥**只在后端使用**：前端点按钮时调用本项目的 `/api/tmdb-search`，由 Flask 用 `requests` 带 key 去请求 TMDB，浏览器里看不到 key。
- 需**联网**才能搜索；搜索功能不可用不影响手动添加记录。
- **未配置 key、搜不到结果、请求失败或超时**都会给出友好提示，不会让页面崩掉。

## 目录结构

```
media-tracker/
├── app.py              # Flask 应用主文件:路由、表单处理、TMDB 代理
...
- 书籍自动补全（Open Library）、动画自动补全（Bangumi）
- 标签
- 用户登录
